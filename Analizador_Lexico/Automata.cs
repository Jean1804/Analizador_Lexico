				...
				switch (_edoAct)
				{
					...
					case 1:
						...
						if ((lenguaje = " \n\r\t").IndexOf(c) >= 0)
							...
							if ((lenguaje = "!\"#$%&\'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]" +
								" ^_'abcdefghijklmnopqrstuvwxyz{|}~€,ƒ„…†‡^‰Š<ŒŽ''˵˶•--~™š>œžŸ " +
								"¡¢£¤¥§¨©ª«¬­®¯°±²³´µ¶·¸¹º»¼½¾¿\f").IndexOf(c) >= 0) _edoAct = 2;
							else
							{
								i = iniToken;
								return false;
					...
					case 4:
						...
						if ((lenguaje = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz").IndexOf(c) >= 0) _edoAct = 4;
							...
							if ((lenguaje = "123456789").IndexOf(c) >= 0) _edoAct = 4;
						else
								if ((lenguaje = "_").IndexOf(c) >= 0) _edoAct = 4;
						else
									if ((lenguaje = "!\"#$%&\'()*+,-./:;<=>?@ABCDEFGHIJKMNOPQRSTUVWXYZ[\\]" +
									"^_`abcdefghijkmnopqrstuvwxy{|}~€‚ƒ„…†‡ˆ‰Š‹ŒŽ''“”•–—˜™š›œžŸ¡¢£¤¥¦§¨©ª«¬-®¯°±²³´µ¶·¸" +
					...
					case 7:
						...
						if ((lenguaje = "0123456789").IndexOf(c) >= 0) _edoAct = 7;
							...
							if ((lenguaje = "!\"#$%&\'()*+,-./:;<=>?@ABCDEFGHIJKMNOPQRSTUVWXYZ[\\]^_`abcdefghijkmnopqrstuvwxy{ | }" +
